        ...
        if st.button("Generate Bar Chart", key="barchart_button"):
            ...
            if text_source == 'Enter Text':
                ...
                with st.spinner("Analyzing entities..."):
                    ...
                    # Count entities to display bar chart
                    ...
                    if len(entity_counts) > 0:
                        ...
                        st.dataframe(df)
                         # Display entities from each URLs with counts
                        st.markdown("### Top 10 Entities Per URL")
                        for url, entity_counts in entity_counts.most_common(10):
                           st.write(f"** {text} (Similarity: {len(entity_counts)}**\n")
                    else:
                        st.warning("No relevant entities found. Please check your text or URL(s).")
            else: #User selected URL
                if not urls:
                    st.warning("Please enter at least one URL.")
                    return
            ...
            with st.spinner("Analyzing entities and generating bar chart..."):
                ...
                if not nlp_model: #Make sure nlp mode was able to run to prevent function to crash later on
                    st.error("Failed to load spaCy model. Aborting.")
                    return

                entity_texts = []
                entity_counts_per_url: Dict[str, Counter] = {}
                for url in urls: #Added all of the content from the different links
                    text = extract_text_from_url(url)
                    entities = identify_entities(text, nlp_model)
                    entity_texts_temp = [entity[0] for entity in entities] #Add all names into a new list
                    entity_counts_per_url[url] = Counter(entity_texts_temp)
                    entity_texts.extend(entity_texts_temp)
                ...
                st.pyplot(fig)
                 ...
                 # Display results in a table
                data = {'Entity': entity_names, 'Count': counts}
                df = pd.DataFrame(data)
                st.dataframe(df)

...
def main():
    ...
    collection_name = st.text_input("Enter your Qdrant collection name:", "my_documents")  # Default value

    # --- Chat Interface ---
    # Initialize chat history and chain.  Do this *outside* the `if prompt:` block
            ...
            if st.session_state.qa_chain:  # Check if qa_chain is initialized
                try:
                    result = st.session_state.qa_chain.invoke(
                        {"input": prompt, "chat_history": [(message["role"], message["content"]) for message in
